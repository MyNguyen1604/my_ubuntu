...
//Struct store Quaternion and Euler
struct Quaterniond
{
  float x;
  float y;
...
float angle_convert(Quaterniond q)
{
  ...
  angle.yaw = atan2(t3, t4);
  //cout<<angle.yaw;

  return angle.yaw;
}

//Calculate distance between initial position and goal
float khoang_cach(float a, float b, float c, float d)
{
  // a,c are x_goal and x_robot respectively
  // b,d are y_goal and y_robot respectively
  float dis;
...
// estimate with diameter = 222
  x = msg->pose.pose.position.x*1000;//*368/400;
  y = msg->pose.pose.position.y*1000;//*368/400;

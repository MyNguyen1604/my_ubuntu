...
private:
  ...
  double gan, xa, vua, nhe, manh, s4, s4_gan, s4_xa, s0, s5, s3, s6, s_max;
  Quaterniond ori;
  bool flag_pp;
...
// estimate with diameter = 222
  ...
  //PATH PLANNING
  x = msg->pose.pose.position.x*100*368/400;
  y = msg->pose.pose.position.y*100*368/400;
...
void Listener::sonarCallback(const sensor_msgs::PointCloud::ConstPtr& pointCloud)
{
  ...
  bool dirty = false;
  bool chi_queo = false;
	s6 = abs(98.96642*(pointCloud->points[6].x))-23.1195;
  s7 = abs(98.96642*(pointCloud->points[7].y))-23.1195;
  s8 = abs(98.96642*(pointCloud->points[8].y))-23.1195;
 	s3 = abs(98.96642*(pointCloud->points[3].x))-23.1195;
  s4 = abs(98.96642*(pointCloud->points[4].x))-23.1195;
  s5 = abs(98.96642*(pointCloud->points[5].x))-23.1195;
  s0 = abs(98.96642*(pointCloud->points[0].y))-23.1195;
  cout << "s4: " << s4 << " s5: " << s5 << " s7: " << s7 << " s8: " << s8 << endl;
  ...
  if (flag_pp == true)
  {
    ...
    if ((s4 < kc_chuyen)||(s5 < kc_chuyen)||(s3 < kc_chuyen))
    {
      flag_pp = false;
      flag_chuyen = true;
    }
  ...
  if (flag_wf == true)
  {
    ...
    if ((delta_theta > 10) && (delta_theta < 30))
     {
      flag_pp = true;
      flag_wf = false;
...
if (flag_wf == true)
{
//  if ((s4 > s4_gan) && (s6 > s4_gan))
	if ((s7 < s_max) && (s8 < s_max) && (s6 > s4_gan))
  {
    if ((s7 < gan) && (s8 < gan))
    {
      angular_ = vua;
